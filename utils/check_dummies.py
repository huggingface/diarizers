...
PATH_TO_DIARIZERS = "src/diarizers"
...
def check_dummies(overwrite=False):
    ...
    path = os.path.join(PATH_TO_DIARIZERS, "utils")
    dummy_file_paths = {
    ...
    for backend in dummy_files.keys():
        if dummy_files[backend] != actual_dummies[backend]:
            if overwrite:
                print(
                    f"Updating diarizers.utils.dummy_{short_names.get(backend, backend)}_objects.py as the main "
                ...
                raise ValueError(
                    ...
                    f"diarizers.utils.dummy_{short_names.get(backend, backend)}_objects.py. Run `make fix-copies` "
                    "to fix this."
                )
